...
if [ "${PACKAGES_ALREADY_INSTALLED}" != "true" ]; then
  ...
  optional_packages="dialog ntp system-lsb-core sysvinit-tools"
  for package in ${optional_packages}; do
    # these can be determined with `yum search`
  ...
  if [[ $(yum list libicu[0-9][0-9] > /dev/null 2>&1) ]]; then
    package_list+=" libicu[0-9][0-9]"
  fi
...
# Restore user dotfile defaults from skeleton file if it doesn't exist or is empty
dotfiles=".bash_profile .bashrc .profile"
for dotfile in ${dotfiles}; do
  if [[ -f "/etc/skel/$dotfile" ]]; then
    if [[ ! -f "${user_rc_path}/$dotfile" ]] || [[ ! -s "${user_rc_path}/$dotfile" ]] ; then
      cp "/etc/skel/$dotfile" "${user_rc_path}/$dotfile"
    fi
  fi
done
